/*
 * p9320_charger.c
 * Samsung P9320 IC Charger Driver
 *
...
static void mfc_set_vout(struct mfc_charger_data *charger, int vout)
{
	...
	pr_info("%s: vout(%s, %d) read = %d mV\n", __func__,
		sb_rx_vout_str(vout), vout, mfc_get_adc(charger, MFC_ADC_VOUT));
...
/* unused function
 * static void mfc_send_multi_packet(
 *	struct mfc_charger_data *charger, u8 header, u8 rx_data_com, u8 *data_val, int data_size)
 * {
 *	int i;
 *
 *	mfc_reg_write(charger->client, MFC_WPC_PCKT_HEADER_REG, header);
...
static void mfc_send_command(struct mfc_charger_data *charger, int cmd_mode)
{
	...
	case MFC_TX_UNO_OFF:
		...
		mfc_send_packet(charger, MFC_HEADER_AFC_CONF, WPC_COM_DISABLE_TX, WPC_UNO_OFF_TX);
		break;
	default:
		break;
	}
...
static void mfc_set_cep_timeout(struct mfc_charger_data *charger, u32 timeout)
{
	if (timeout) {
		...
		pr_info("%s: timeout = %ds\n", __func__, timeout);

		data[0] = timeout & 0xFF;
		data[1] = (timeout & 0xFF00) >> 8;
		mfc_reg_write(charger->client, MFC_TX_CEP_TIMEOUT_L, data[0]);
		mfc_reg_write(charger->client, MFC_TX_CEP_TIMEOUT_H, data[1]);
	}
}
...
static void mfc_tx_handle_rx_packet(struct mfc_charger_data *charger)
{
	...
	if (cmd_data == MFC_HEADER_AFC_CONF) { /* 0x28 */
		...
		case WPC_COM_RX_ID: /* 0x0E Received RX ID */
			if (val2_data >= RX_DATA_VAL2_RXID_ACC_BUDS &&
				val2_data <= RX_DATA_VAL2_RXID_ACC_BUDS_MAX) {	/* Buds */
		...
		if (val1_data == MFC_EPT_CODE_OVER_TEMPERATURE) {
			...
			psy_do_property("wireless", set, POWER_SUPPLY_EXT_PROP_WIRELESS_TX_ERR, value);
		} else if (val1_data == MFC_EPT_CODE_BATTERY_FAILURE) {
...
static void mfc_wpc_afc_vout_work(struct work_struct *work)
{
	...
	if (charger->pdata->cable_type == SEC_BATTERY_CABLE_PREPARE_WIRELESS_HV) {
		if (charger->is_abnormal_pad) {
			...
			charger->pdata->cable_type = value.intval = SEC_BATTERY_CABLE_HV_WIRELESS;
		}
	} else {
...
static void mfc_wpc_fw_update_work(struct work_struct *work)
{
	...
	case SEC_WIRELESS_RX_BUILT_IN_MODE:
		...
		if (charger->fw_cmd == SEC_WIRELESS_RX_SPU_MODE ||
			charger->fw_cmd == SEC_WIRELESS_RX_SPU_VERIFY_MODE) {
			if (spu_firmware_signature_verify("MFC",
				charger->firm_data_bin->data, charger->firm_data_bin->size) ==
		...
		__pm_stay_awake(charger->wpc_update_ws);
		pr_info("%s: data size = %ld, chip_id(%d)\n", __func__, (long)fw_size, charger->chip_id);
...
static void mfc_set_CMB(struct mfc_charger_data *charger, bool enable)
{
	pr_info("%s: enable(%d)\n", __func__, enable);
	...
	if (enable) {
		/* ENABLE CMB */
		mfc_reg_write(charger->client, MFC_RX_COMM_MOD_AFC_FET_REG,
			CMA_ENABLE_CMB_ENABLE_DATA);
		mfc_reg_write(charger->client, MFC_RX_COMM_MOD_FET_REG,
			CMA_ENABLE_CMB_ENABLE_DATA);
	} else {
		/* DISABLE CMB */
		mfc_reg_write(charger->client, MFC_RX_COMM_MOD_AFC_FET_REG,
			CMA_ENABLE_CMB_DISABLE_DATA);
		mfc_reg_write(charger->client, MFC_RX_COMM_MOD_FET_REG,
			CMA_ENABLE_CMB_DISABLE_DATA);
	}
}
...
static void mfc_check_high_temp_swelling(struct mfc_charger_data *charger, bool is_charging)
{
	...
	if (is_charging) {
		...
		if (capacity > 70)
			mfc_set_CMB(charger, true);
	} else {
		/* Set CS100 FOD */
		mfc_fod_set(charger, FOD_STATE_FULL);
		mfc_set_CMB(charger, false);
	}
}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		if (charger->wc_tx_enable) {
			...
			if (mfc_reg_multi_read(charger->client, MFC_WPC_FOD_0A_REG, fod, 12) < 0)
				pr_info("%s: failed to read FOD data\n", __func__);
			else
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_WIRELESS_TIMER_ON:
			...
			if (charger->wc_tx_enable) {
				if (val->intval) {
					...
					mfc_reg_update(charger->client, MFC_INT_A_ENABLE_H_REG,
							MFC_INTA_H_TX_MODE_RX_NOT_DET_MASK,
							MFC_INTA_H_TX_MODE_RX_NOT_DET_MASK);
				} else {
		...
		case POWER_SUPPLY_EXT_PROP_INPUT_VOLTAGE_REGULATION:
			if (val->intval == WIRELESS_VOUT_NORMAL_VOLTAGE) {
				...
				cancel_delayed_work(&charger->wpc_vout_mode_work);
				__pm_stay_awake(charger->wpc_vout_mode_ws);
				queue_delayed_work(charger->wqueue, &charger->wpc_vout_mode_work,
...
static void mfc_wpc_opfq_work(struct work_struct *work)
{
	...
	if ((pad_mode == PAD_MODE_WPC_BASIC) ||	(pad_mode == PAD_MODE_WPC_ADV)) {
		...
		charger->pdata->cable_type = value.intval = SEC_BATTERY_CABLE_PMA_WIRELESS;
		psy_do_property("wireless", set, POWER_SUPPLY_PROP_ONLINE, value);
	}
...
static void mfc_wpc_tx_isr_work(struct work_struct *work)
{
	...
	if (!charger->wc_tx_enable || !charger->wc_rx_connected) {
		...
		if (!charger->wc_tx_enable || (cmd_data != MFC_HEADER_END_POWER_TRANSFER)) {
			__pm_relax(charger->wpc_tx_ws);
			return;
		}
	}
...
static void mfc_wpc_isr_work(struct work_struct *work)
{
	...
	if (cmd_data == WPC_TX_COM_AFC_SET) {
		...
		case TX_AFC_SET_10V:
			...
			/* change cable type */
			charger->pdata->cable_type = value.intval = SEC_BATTERY_CABLE_PREPARE_WIRELESS_HV;
			psy_do_property("wireless", set, POWER_SUPPLY_PROP_ONLINE, value);
				...
				if (charger->pad_vout == PAD_VOUT_10V) {
					if (charger->pdata->cable_type == SEC_BATTERY_CABLE_PREPARE_WIRELESS_HV) {
						...
						charger->pdata->cable_type =
							value.intval = SEC_BATTERY_CABLE_WIRELESS_HV_VEHICLE;
					}
				} else {
			...
			case TX_ID_STAND_TYPE_START:
				if (charger->pad_vout == PAD_VOUT_10V) {
					if (charger->pdata->cable_type == SEC_BATTERY_CABLE_PREPARE_WIRELESS_HV) {
						...
						charger->pdata->cable_type =
							value.intval = SEC_BATTERY_CABLE_WIRELESS_HV_STAND;
					}
				} else {
					charger->pdata->cable_type = value.intval = SEC_BATTERY_CABLE_WIRELESS_STAND;
				}
	...
	if (!np) {
		...
		if (p) {
			...
			pdata->wireless20_vrect_list =
					kzalloc(sizeof(*pdata->wireless20_vrect_list) * len, GFP_KERNEL);
		...
		if (p) {
			...
			pdata->wireless20_max_power_list =
				kzalloc(sizeof(*pdata->wireless20_max_power_list) * len, GFP_KERNEL);
...
static int __init mfc_charger_init(void)
{
	...
	if (sb_mfc_check_chip_id(MFC_CHIP_ID_P9320) == CHIP_ID_NOT_SET)
		return 0;
