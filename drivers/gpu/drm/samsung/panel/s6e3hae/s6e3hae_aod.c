/*
 * linux/drivers/video/fbdev/exynos/panel/usdm/s6e3hae/s6e3hae_aod.c
 *
...
int s6e3hae_init_self_mask_ctrl(struct maptbl *tbl)
{
	...
	props->self_mask_crc_len = SELFMASK_CHECKSUM_LEN;
	props->self_mask_crc = kmalloc(sizeof(u8) * props->self_mask_crc_len, GFP_KERNEL);
	if (!props->self_mask_crc) {
		panel_err("failed to mem alloc\n");
		return -ENOMEM;
	}
	props->self_mask_crc[0] = SELFMASK_CHECKSUM_VALID1;
	props->self_mask_crc[1] = SELFMASK_CHECKSUM_VALID2;
	panel_info("was called\n");
	return 0;
}
