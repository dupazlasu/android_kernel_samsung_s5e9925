...
{
	int row = 0, layer = S6E3HAE_LPM_LFD_1HZ, lpm_lfd_min_freq;
	struct panel_device *panel;
	struct panel_bl_device *panel_bl;
	struct panel_properties *props;
	struct vrr_lfd_config *vrr_lfd_config;
	...
	if (row < 0) {
		panel_err("failed to get AOD brightness index(%d)\n", row);
		return -EINVAL;
	}
	...
	if (vrr_lfd_config->fix == VRR_LFD_FREQ_HIGH ||
		vrr_lfd_config->fix == VRR_LFD_FREQ_HIGH_UPTO_SCAN_FREQ) {
		layer = S6E3HAE_LPM_LFD_30HZ;
		vrr_lfd_status->lfd_min_freq = 30;
		vrr_lfd_status->lfd_min_freq_div = 1;
		panel_info("lpm_fps %dhz (layer:%d)\n",
				(layer == S6E3HAE_LPM_LFD_1HZ) ? 1 : 30, layer);
...
struct pnobj_func s6e3hae_function_table[MAX_S6E3HAE_FUNCTION] = {
	...
	[S6E3HAE_GETIDX_LPM_BRT_TABLE] = __PNOBJ_FUNC_INITIALIZER(S6E3HAE_GETIDX_LPM_BRT_TABLE, s6e3hae_getidx_lpm_brt_table),
