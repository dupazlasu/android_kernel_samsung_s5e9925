...
#ifdef CONFIG_SEC_PM
	regulator_init_ws = wakeup_source_register(NULL, "regulator_init_ws");
	if (regulator_init_ws)
		__pm_wakeup_event(regulator_init_ws, 35000);
	else
		pr_err("%s: Fail to register wakeup_soruce\n", __func__);
#endif /* CONFIG_SEC_PM */
