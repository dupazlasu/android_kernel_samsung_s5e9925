...
static int __exynos_pm_qos_show(struct exynos_pm_qos_object *qos, char *buf)
{
	...
	plist_for_each_entry(req, &c->list, node) {
		...
		tot_reqs++;
		size += snprintf(buf + size, PAGE_SIZE, "%d: %d: %s(%s:%d) (%llu.%llu)\n", tot_reqs,
	...
	size += snprintf(buf + size, PAGE_SIZE, "Type=%s, Value=%d, Requests: active=%d / total=%d, time=%llu.%llu\n",
		   type, exynos_pm_qos_get_value(c), active_reqs, tot_reqs,
...
static int register_pm_qos_misc(struct exynos_pm_qos_object *qos, struct dentry *d, struct kobject *kobj)
{
	...
	sysfs_create_file_ns(kobj, &qos->kobj_attr.attr, NULL);
	...
	sysfs_create_bin_file(kobj, &qos->bin_attr);
